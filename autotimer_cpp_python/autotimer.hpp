 ...
 * Last modified: 2022-07-19 21:18:00
 *
 * Usage:
 *
...
#if __cplusplus >= 201103L || (defined(_MSC_VER) && _MSC_VER >= 1900) // C++11 required
#include <chrono>

class AdvanceTimer
{
public:
    AdvanceTimer(const std::string& name, int loopCount = 1, bool autoReport = true)
        : mName(name), mLoopCount(loopCount), mAutoReport(autoReport)
    {
        mStartTime = std::chrono::system_clock::now();
    }
    static std::chrono::duration<long, std::ratio<1, 1000000>>::rep getElapsedTime(std::chrono::system_clock::time_point startTime)
    {
        auto now = std::chrono::system_clock::now();
        return std::chrono::duration_cast<TimeUnit>(now - startTime).count();
    }
    void report() const
    {
        auto totalCost = getElapsedTime(mStartTime);
        //std::cout << mName << " took "<< totalCost << TimeUnitStr;
        fprintf(stderr, "%s took %ld %s", mName.c_str(), totalCost, TimeUnitStr.c_str());
        if (mLoopCount > 1)
        {
            //std::cout << ", loop = " << mLoopCount << ", avg = " << totalCost / (mLoopCount*1.0) << TimeUnitStr << std::endl;
            fprintf(stderr, ", loop = %d, avg = %.2f %s", mLoopCount, totalCost / (mLoopCount * 1.0), TimeUnitStr.c_str());
        }
        fprintf(stderr, "\n");
    }
    ~AdvanceTimer()
...
private:
    ...
    // TODO: how do we put TimeUnit as template argument, and deduce TimeUnitStr's value?
    using TimeUnit = std::chrono::microseconds;
    const std::string TimeUnitStr = "us";

    // using TimeUnit = std::chrono::milliseconds;
    // const std::string TimeUnitStr = "ms";
};
